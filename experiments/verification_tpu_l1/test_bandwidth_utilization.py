    ...
    for n_it in range(n_tile_num):
        for k_it in range(k_tile_num):
            for m_it_st in range(0, m_tile_num, load_burst_len):
                ...
                if m_it_st == 0:
                    core.async_buffer_read(cb_wgt_ptr[0], bf_wgt_ptr[n_it * k_tile_num + k_it])
                core.async_buffer_read(cb_ifm_ptr[:m_it_n_pages], bf_ifm_ptr[k_it * m_tile_num + m_it_st : k_it * m_tile_num + m_it_ed])
                core.async_buffer_read(cb_psum_ptr[:m_it_n_pages], psum_tile_bf_ptr[:m_it_n_pages])
                ...
                core.async_buffer_write(bf_ofm_ptr[n_it * m_tile_num + m_it_st:n_it * m_tile_num + m_it_ed], cb_ofm_ptr[:m_it_n_pages])
                core.async_rpc_wait_all()
