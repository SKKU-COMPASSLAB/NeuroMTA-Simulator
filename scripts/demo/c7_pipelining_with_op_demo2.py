...
if __name__ == "__main__":
    ...
    relu_core_group = device.get_npu_core_group((4, 0), (4, 4))
    ...
    ifm  = torch.randint(-16, 16, (M, K), dtype=dtype).reshape(M, K)
    wgt  = torch.randint(-16, 16, (N, K), dtype=dtype).reshape(N, K)
    bias = torch.randint(-16, 16, (N,), dtype=acc_dtype)
    ...
    spad_ld_pp_space_relu = device.create_l1_mem_space(parse_mem_cap_str("192KB"), core_group=relu_core_group)
    spad_st_pp_space_relu = device.create_l1_mem_space(parse_mem_cap_str("32KB"), core_group=relu_core_group)
    

    ifm1_b  = MCA_TensorBuffer(mem_space=ifm_mem_space1,  shape=ifm.shape,  dtype=ifm.dtype,  shard_shape=(1,  32)).allocate().update(ifm)
    wgt1_b  = MCA_TensorBuffer(mem_space=param_mem_space, shape=wgt.shape,  dtype=wgt.dtype,  shard_shape=(32, 32)).allocate().update(wgt)
    bias1_b = MCA_TensorBuffer(mem_space=param_mem_space, shape=bias.shape, dtype=bias.dtype, shard_shape=(1,  32)).allocate().update(bias)
    
    ifm2_b  = MCA_TensorBuffer(mem_space=ofm_mem_space1,  shape=ofm.shape,  dtype=ofm.dtype,  shard_shape=(1,  32)).allocate()
    wgt2_b  = MCA_TensorBuffer(mem_space=param_mem_space, shape=wgt.shape,  dtype=acc_dtype,  shard_shape=(32, 32)).allocate().update(wgt.to(acc_dtype))
    bias2_b = MCA_TensorBuffer(mem_space=param_mem_space, shape=bias.shape, dtype=bias.dtype, shard_shape=(1,  32)).allocate().update(bias)
    
    ofm_b   = MCA_TensorBuffer(mem_space=ofm_mem_space2,  shape=ofm.shape,  dtype=ofm.dtype,  shard_shape=(1,  32)).allocate()
    ...
    operator2 = MCA_OP_RELU_INPLACE(
        device=device,
        core_group=relu_core_group,
        spad_ld_mem_space=spad_ld_pp_space_relu,
        spad_st_mem_space=spad_st_pp_space_relu,
        ifm=ifm2_b,
    )
    
    operator3 = MCA_OP_LINEAR(
        device=device,
    ...
    with MonitoringWindow() as monitor:
        for core_id in core_group1.core_ids:
            ...
            pbar_idx = monitor.add_core_pbar(desc=f"GRP 1 {core_id:<3d}", ncols=60)
            monitor.pbar_handles[pbar_idx].bind_core(core)
            
        for core_id in core_group2.core_ids:
            core = device.get_npu_core(core_id=core_id)
            pbar_idx = monitor.add_core_pbar(desc=f"GRP 2 {core_id:<3d}", ncols=60)
            monitor.pbar_handles[pbar_idx].bind_core(core)
            
        for core_id in relu_core_group.core_ids:
            core = device.get_npu_core(core_id=core_id)
            pbar_idx = monitor.add_core_pbar(desc=f"RELU  {core_id:<3d}", ncols=60)
            monitor.pbar_handles[pbar_idx].bind_core(core)
    ...
    reference = torch.nn.functional.relu(reference)
    reference = torch.matmul(reference, wgt.t().to(acc_dtype)) + bias
