...
def main():
    ...
    for step in range(1, x * y, 1):
        for dst_id in range(1, step):
            test_context.dispatch_single_cmd(src_id=dst_id-1, dst_id=dst_id, packet_size=packet_size)
        test_context.run_test()
    
    os.makedirs(save_dir, exist_ok=True)

    summary_file_name = f"topo_{x}x{y}x{xr}x{yr}_subnet_{subnets}.csv"
