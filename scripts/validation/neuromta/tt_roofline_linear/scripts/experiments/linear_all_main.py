...
class Benchmark:
    ...
    def run(self, device):
        ...
        tt_ifm  = ttnn.from_torch(ifm,  dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device, memory_config=ttnn.DRAM_MEMORY_CONFIG)
        ...
        for i in range(warmup_iters + test_iters):
            ...
            matmul_result = ttnn.linear(tt_ifm, tt_wgt, bias=tt_bias, memory_config=ttnn.DRAM_MEMORY_CONFIG, core_grid=ttnn.CoreGrid(y=4, x=4))
            if not is_warmup:
