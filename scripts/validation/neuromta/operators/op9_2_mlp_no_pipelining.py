...
if __name__ == "__main__":
    ...
    x_b            = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=x.shape,            dtype=x.dtype).allocate().update(x)
    linear1_wgt_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear1_wgt.shape,  dtype=linear1_wgt.dtype).allocate().update(linear1_wgt)
    linear1_bias_b = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear1_bias.shape, dtype=linear1_bias.dtype).allocate().update(linear1_bias.unsqueeze(0))
    linear1_ofm_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear1_ofm.shape,  dtype=linear1_ofm.dtype).allocate()
    relu1_ofm_b    = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=relu1_ofm.shape,    dtype=relu1_ofm.dtype).allocate()
    linear2_wgt_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear2_wgt.shape,  dtype=linear2_wgt.dtype).allocate().update(linear2_wgt)
    linear2_bias_b = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear2_bias.shape, dtype=linear2_bias.dtype).allocate().update(linear2_bias.unsqueeze(0))
    linear2_ofm_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear2_ofm.shape,  dtype=linear2_ofm.dtype).allocate()
    relu2_ofm_b    = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=relu2_ofm.shape,    dtype=relu2_ofm.dtype).allocate()
    linear3_wgt_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear3_wgt.shape,  dtype=linear3_wgt.dtype).allocate().update(linear3_wgt)
    linear3_bias_b = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear3_bias.shape, dtype=linear3_bias.dtype).allocate().update(linear3_bias.unsqueeze(0))
    linear3_ofm_b  = MCA_TensorBuffer(mem_space=main_data_mem_space, shape=linear3_ofm.shape,  dtype=linear3_ofm.dtype).allocate()
