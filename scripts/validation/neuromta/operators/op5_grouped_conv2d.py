...
if __name__ == "__main__":
    ...
    global_recipe=MCA_OperatorGraphCompiler.CompileRecipe(
        ...
        # temporal_reuse_target=MCA_OperatorGraphCompiler.CompileRecipe.ReuseTarget.SINGLE_L1,       # ifm temporal reuse
        # spatial_reuse_target=MCA_OperatorGraphCompiler.CompileRecipe.ReuseTarget.SINGLE_MAIN,   # weight broadcast
    )
    ...
    if args.report_mismatch:
        if not torch.equal(simulated, reference):
            mismatch_report = os.path.join(SUMMARY_DIR, "grouped_conv_mismatch_report.txt")
