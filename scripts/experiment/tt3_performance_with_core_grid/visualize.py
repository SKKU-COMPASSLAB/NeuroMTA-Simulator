...
PEAK_PERFORMANCE = 2 * (4 * 8) * (32 * 32)  # 4x4 Core Grid | 32x32 MXU | MAC = 2 OPs 
...
def draw(peak_perf: int, peak_bw: int, src_path: str, img_path: str, img_title: str):
    ...
    for idx, row in df.iterrows():
        ...
        mnk_dims = name.split('_')[0] # extract only MNK dimensions
        grid_shape = name.split('_')[-1] # extract only core grid shape
        name = f"{mnk_dims}_{grid_shape}"
