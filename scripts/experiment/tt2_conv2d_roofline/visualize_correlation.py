...
def sim_bench_name(sim_bench: str) -> str:
    img_shape = sim_bench.split("_")[3]
    kernel_shape = sim_bench.split("_")[4]
    
    batch_size = img_shape.split("x")[0]
    img_size = "x".join(img_shape.split("x")[1:3])
    in_channels = img_shape.split("x")[3]
    out_channels = kernel_shape.split("x")[2]
    filter_size = "x".join(kernel_shape.split("x")[0:2])
    
    return f"B{batch_size}_{img_size}_C{in_channels}_Cout{out_channels}_K{filter_size}"

def ref_bench_name(ref_bench: str) -> str:
    return "_".join(ref_bench.split("_")[1:6])
...
def get_data(sim_path: str, ref_path: str) -> dict[str, tuple[float, float]]:
    ...
    for (_, sim_row), (_, ref_row) in zip(sim_df.iterrows(), ref_df.iterrows()):
        sim_bench = sim_bench_name(sim_row["Benchmark"])
        ref_bench = ref_bench_name(ref_row["benchmark"])
