...
class Benchmark:
    ...
    def run(self, device: TenstorrentDevice, core_group: MTA_CoreGrid):
        ...
        ifm_b  = MCA_TensorBuffer(mem_space=l1_data_mem_space,   shape=ifm.shape,         dtype=ifm.dtype,       shard_shape=(Mt, Kt), blocked_mapping=True).allocate().update(ifm)
        ...
        ofm_b  = MCA_TensorBuffer(mem_space=l1_data_mem_space,   shape=(self.M, self.N),  dtype=self.acc_dtype,  shard_shape=(Mt, Nt), blocked_mapping=True).allocate()
