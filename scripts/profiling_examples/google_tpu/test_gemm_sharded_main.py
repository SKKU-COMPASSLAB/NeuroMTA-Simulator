...
if __name__ == "__main__":
    ...
    n_threads = 2  # assumes double buffering
    ...
    spm_ifm_ptr:  Reference = device.create_local_l1_buffer(page_size=ifm_tile_size, n_pages=n_threads, core_ids=npu_core_id)
    spm_wgt_ptr:  Reference = device.create_local_l1_buffer(page_size=wgt_tile_size, n_pages=n_threads, core_ids=npu_core_id)
    spm_psum_ptr: Reference = device.create_local_l1_buffer(page_size=ofm_tile_size, n_pages=n_threads, core_ids=npu_core_id)
    spm_ofm_ptr:  Reference = device.create_local_l1_buffer(page_size=ofm_tile_size, n_pages=n_threads, core_ids=npu_core_id)
    ...
    print(f"simulation terminated with {device.timestamp} (may be preterminated due to profiler convergence)")