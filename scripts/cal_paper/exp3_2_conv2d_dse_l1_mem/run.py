...
class Conv2dBenchmark(Benchmark):
    ...
    def signature(self) -> str:
        ...
        return f"CV_C{self.n_cores}_{ifm_shape_signature}_{wgt_shape_signature}_{stride_signature}_{padding_signature}_{dilation_signature}_LDEXST{self.ld_ex_buffer_size + self.ex_st_buffer_size}K_BCAST{self.bcast_buffer_size}K_CACHE{self.cache_buffer_size}K"
...
def bcast_queue_depth(cache_slot_num: int) -> int:
    ...
    return remainings // 7 * 4

def ld_ex_st_buffer_slot_num(cache_slot_num: int) -> int:
    remainings = total_n_slots - cache_slot_num
    return remainings // 7 * 3

benchmarks = [
    Conv2dBenchmark(
        ld_ex_st_buffer_slot_num=ld_ex_st_buffer_slot_num(cache_slot_num),
        bcast_optimize_queue_depth=bcast_queue_depth(cache_slot_num),
        cache_slot_num=cache_slot_num
    )
    for cache_slot_num in cache_slot_nums
]
