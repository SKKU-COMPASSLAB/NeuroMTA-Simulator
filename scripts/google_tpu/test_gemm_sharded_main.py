...
def compute_kernel(
    ...
    core.sem_create(load_sem, initial_value=0)
    core.sem_create(mxu_sem, initial_value=0)
    core.sem_create(store_sem, initial_value=0)
    
    for thread_id in range(n_threads):
        core.sem_create(thread_sem[thread_id], initial_value=0)
    ...
    for n_it in range(n_tile_num):
        ...
        with new_parallel_thread():
            core.sem_compare_and_set(thread_sem[thread_id], 0, 1)
            
            for k_it in range(k_tile_num):
                core.sem_compare_and_set(load_sem, 0, 1)
                core.async_buffer_read(spm_wgt_ptr[thread_id], bf_wgt_ptr[n_it * k_tile_num + k_it])
                core.sem_compare_and_set(load_sem, 1, 0)
                ...
                for m_it in range(m_tile_num):
                    ...
                    core.sem_compare_and_set(mxu_sem, 0, 1)
                    core.mxu_tiled_gemm(
                    ...
                    core.sem_compare_and_set(mxu_sem, 1, 0)

                    core.sem_compare_and_set(store_sem, 0, 1)
                    core.async_buffer_write(bf_ofm_ptr[n_it * m_tile_num + m_it], spm_ofm_ptr[thread_id])
                    core.sem_compare_and_set(store_sem, 1, 0)
                    
            core.sem_compare_and_set(thread_sem[thread_id], 1, 0)
