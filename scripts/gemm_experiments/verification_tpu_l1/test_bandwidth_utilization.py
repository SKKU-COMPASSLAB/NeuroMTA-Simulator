            ...
            for m_it_st in range(0, m_tile_num, load_burst_len):
                ...
                if m_it_st == 0:
                    with new_parallel_thread():
                        ...
                        core.local_memcopy_buffer(cb_wgt_ptr[0], bf_wgt_ptr[n_it * k_tile_num + k_it])
                        core.cb_push_back(cb_wgt_ptr, n_pages=1)

                with new_parallel_thread():
                    core.cb_reserve_back(cb_ifm_ptr, n_pages=m_it_n_pages)
                    core.local_memcopy_buffer(cb_ifm_ptr[:m_it_n_pages], bf_ifm_ptr[k_it * m_tile_num + m_it_st : k_it * m_tile_num + m_it_ed])
                    core.cb_push_back(cb_ifm_ptr, n_pages=m_it_n_pages)
                
                with new_parallel_thread():
                    core.cb_reserve_back(cb_psum_ptr, n_pages=m_it_n_pages)
                    core.local_memcopy_buffer(cb_psum_ptr[:m_it_n_pages], psum_tile_bf_ptr[:m_it_n_pages])
                ...
                with new_parallel_thread():
                    ...
                    core.local_memcopy_buffer(bf_ofm_ptr[n_it * m_tile_num + m_it_st:n_it * m_tile_num + m_it_ed], cb_ofm_ptr[:m_it_n_pages])
    ...
    for n_it in range(n_tile_num):
        for k_it in range(k_tile_num):            
            for m_it in range(m_tile_num):
                ...
                core.mxu_tiled_gemm(
                    cb_ifm_ptr[0], cb_wgt_ptr[0], cb_psum_ptr[0], cb_ofm_ptr[0], 
                    preload_wgt=(m_it == 0),
                    preload_psum=False,
                    flush_ofm=True,
                )
