...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_LINEAR(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_RELU(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_MERGED_LINEAR_RELU(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
...
@jit_prototype    
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_CONV2D(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_MAXPOOL2D(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_TILED_AVGPOOL2D(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
...
@jit_prototype
def MCA_KERNEL_CORE_STAGE_COMPUTE_DIRECT_COPY(core: NPUCore, env: MCA_OperatorGraphCompiler.Environment, cmd: MCA_CompiledOperator.IR.Base):
    if isinstance(cmd, MCA_CompiledOperator.IR.EXE_UOP):
        op_sig = env.op_meta[cmd.op_id].op_sig
        tiled_op = op_sig.tiled_ops[cmd.tiled_op_idx]
