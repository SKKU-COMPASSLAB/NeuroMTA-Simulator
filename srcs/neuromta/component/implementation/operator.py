...
_global_mca_op_options = {
    "spad_ld_mem_space_size": None,
    "spad_st_mem_space_size": None,
}

def set_global_mca_op_option(spad_ld_mem_space_size: int=None, spad_st_mem_space_size: int=None):
    global _global_mca_op_options
    
    if spad_ld_mem_space_size is not None:
        _global_mca_op_options["spad_ld_mem_space_size"] = spad_ld_mem_space_size
    if spad_st_mem_space_size is not None:
        _global_mca_op_options["spad_st_mem_space_size"] = spad_st_mem_space_size

def get_global_mca_op_option(option_name: str) -> Any:
    global _global_mca_op_options
    
    if option_name not in _global_mca_op_options:
        raise ValueError(f"Invalid global MCA operator option name: {option_name}")
    return _global_mca_op_options[option_name]


def mca_operator_method(func: Callable):
    def _mca_mapper_method_wrapper(*args, **kwargs) -> 'MCA_OperatorSignature':
        spad_ld_mem_space_size = kwargs.pop("spad_ld_mem_space_size", get_global_mca_op_option("spad_ld_mem_space_size"))
        spad_st_mem_space_size = kwargs.pop("spad_st_mem_space_size", get_global_mca_op_option("spad_st_mem_space_size"))
        
        if spad_ld_mem_space_size is None or spad_st_mem_space_size is None:
            raise ValueError("SPAD memory space sizes must be specified either as arguments to the decorated function or as global MCA operator options.")
        ...
        op_sig.update_spad_mem_space_sizes(spad_ld_mem_space_size, spad_st_mem_space_size)
        return op_sig
    return _mca_mapper_method_wrapper


class MCA_OperatorSignature:
    def __init__(self, op_type: str, op_template: Callable, op_ex_kernels: list[Callable], spad_ld_mem_space_size: int=None, spad_st_mem_space_size: int=None):
        if spad_ld_mem_space_size is None:
            spad_ld_mem_space_size = get_global_mca_op_option("spad_ld_mem_space_size")
        if spad_st_mem_space_size is None:
            spad_st_mem_space_size = get_global_mca_op_option("spad_st_mem_space_size")
    ...
    def update_spad_mem_space_sizes(self, spad_ld_mem_space_size: int=None, spad_st_mem_space_size: int=None):
        if spad_ld_mem_space_size is not None:
            self.spad_ld_mem_space_size = spad_ld_mem_space_size
            self.spad_ld_pp_mem_space_size = spad_ld_mem_space_size // 2
    ...
    def initialize_core_group(self, core_group: MCA_CoreGroup, spad_ld_mem_space: MCA_L1MemorySpace, spad_st_mem_space: MCA_L1MemorySpace):
        if not isinstance(spad_ld_mem_space, MCA_L1MemorySpace):
            raise ValueError("spad_ld_mem_space must be an instance of MCA_L1MemorySpace.")
        if not isinstance(spad_st_mem_space, MCA_L1MemorySpace):
            raise ValueError("spad_st_mem_space must be an instance of MCA_L1MemorySpace.")
        ...
        self.spad_ld_pp_ptrs = {
            core_id: (
                spad_ld_mem_space.allocate(core_id, self.spad_ld_pp_mem_space_size),
                spad_ld_mem_space.allocate(core_id, self.spad_ld_pp_mem_space_size),
            )
            for core_id in spad_ld_mem_space.owner_ids
        }
        
        # ping-pong pointers for store
        self.spad_st_pp_ptrs = {
            core_id: (
                spad_st_mem_space.allocate(core_id, self.spad_st_pp_mem_space_size),
                spad_st_mem_space.allocate(core_id, self.spad_st_pp_mem_space_size),
            )
            for core_id in spad_st_mem_space.owner_ids
        }
...
class MCA_CompiledOperatorGraph:
    ...
    def summary(self) -> dict:
        ...
        for core_id, stages in self._mappings.items():
            ...
            for stage in stages:
                stage_summary = {
                    "preprocessing":  [cmd.signature() for cmd in stage.preprocessing_commands  if not isinstance(cmd, MCA_CompiledOperatorGraph.Command.NOP)],
                    "mem_load":       [cmd.signature() for cmd in stage.mem_load_commands       if not isinstance(cmd, MCA_CompiledOperatorGraph.Command.NOP)],
                    "execute":        [cmd.signature() for cmd in stage.execute_commands        if not isinstance(cmd, MCA_CompiledOperatorGraph.Command.NOP)],
                    "mem_store":      [cmd.signature() for cmd in stage.mem_store_commands      if not isinstance(cmd, MCA_CompiledOperatorGraph.Command.NOP)],
                    "postprocessing": [cmd.signature() for cmd in stage.postprocessing_commands if not isinstance(cmd, MCA_CompiledOperatorGraph.Command.NOP)],
                }
...
class MCA_OperatorGraphCompiler:
    ...
    class OperatorRecipe:
        def __init__(
            self,
            spatial_reuse_target_buf_idx: int,
            use_broadcast_optimize: bool,
        ):
    ...
    class GlobalRecipe:
        def __init__(
            ...
            if MCA_OperatorGraphCompiler.DEFAULT not in self.op_recipes:
                self.op_recipes[MCA_OperatorGraphCompiler.DEFAULT] = MCA_OperatorGraphCompiler.OperatorRecipe(
        ...
        def get_operator_recipe(self, op_sig: MCA_OperatorSignature) -> 'MCA_OperatorGraphCompiler.OperatorRecipe':
            ...
            return self.op_recipes.get(op_sig.op_type, self.op_recipes[MCA_OperatorGraphCompiler.DEFAULT])
        
        @property
        def max_n_operators(self) -> int:
    ...
    def _compile_initialize(self, global_recipe: 'MCA_OperatorGraphCompiler.GlobalRecipe', target_ops: list[str]) -> 'MCA_OperatorGraphCompiler.Environment':
        ...
        _n_cores_weight_per_op: dict[str, int] = {}
        _total_n_core_groups_available: int = len(global_recipe.core_groups)
        
        for op_id in target_ops:
            op_sig = self._op_sigs[op_id]
            _n_cores_weight_per_op[op_id] = 0
            for tiled_op in op_sig.tiled_ops:
                _n_cores_weight_per_op[op_id] += len(tiled_op.i_tiles)
            
        _n_core_groups_per_op = self._distribute_resources(_total_n_core_groups_available, _n_cores_weight_per_op)
        _core_group_alloc_offset = 0
        
        for op_id in target_ops:
            op_sig = self._op_sigs[op_id]
            n_core_groups = _n_core_groups_per_op[op_id]
            ...
            spad_ld_mem_space = global_recipe.device.create_l1_mem_space(
                size_per_bank=op_sig.spad_ld_mem_space_size,
                core_group=core_group,
            )
            
            spad_st_mem_space = global_recipe.device.create_l1_mem_space(
                size_per_bank=op_sig.spad_st_mem_space_size,
                core_group=core_group,
            )
            
            op_sig.initialize_core_group(core_group, spad_ld_mem_space, spad_st_mem_space)
            logger.debug(f"Initialized core group for operator {op_id}: {core_group}")
            
            spad_st_mem_space.remove()  # remove temporary memory space used for core group initialization, since each operator signature now has its own memory space allocated for ping-pong buffers.
            spad_ld_mem_space.remove()  # remove temporary memory space used for core group initialization, since each operator signature now has its own memory space allocated for ping-pong buffers.
    ...
    def _compile_create_mapping(self, global_recipe: 'MCA_OperatorGraphCompiler.GlobalRecipe', target_ops: list[str]) -> dict[str, dict[int, list[int]]]:
        ...
        for op_id in target_ops:
            ...
            if global_recipe.get_operator_recipe(op_sig).use_broadcast_optimize:
                ...
                for idx, tiled_op in enumerate(op_sig.tiled_ops):
                    core_id = core_ids[idx % n_cores]
                    clustered_tiled_ops[op_id][core_id].append(idx)
                            ...
                            if global_recipe.get_operator_recipe(op_sig).use_broadcast_optimize and uop_idx != 0:
                                ...
                                current_stage.mem_store_commands.append(
                                    MCA_CompiledOperatorGraph.Command.MEM_STORE_TILE(
                                        ...
                                        tile_sig=o_tile,
                                        ptr=st_ptr,
                                        is_partial=True,  # partial store since we only need to store the context part of the tile
                                    )
                                )
    ...
    def _compile_pipeline_optimize(self, global_recipe: 'MCA_OperatorGraphCompiler.GlobalRecipe', env: 'MCA_OperatorGraphCompiler.Environment', compiled_ops: 'dict[str, MCA_CompiledOperatorGraph]') -> dict[str, MCA_CompiledOperatorGraph]:
        ...
        for dst_op_idx in range(1, n_ops):
            ...
            is_pipeline_detected = False
            
            for src_op_idx in reversed(range(dst_op_idx)):
                ...
                if is_pipeline_detected:
                    break
                elif len(common_buf_names) == 0:
                    continue
                else:
                    is_pipeline_detected = True
                    
                logger.debug(f"  >> Pipeline detected between {src_op_id} and {dst_op_id} with common buffers: {common_buf_names}")
                ...
                for src_stage_cursor in range(n_src_stages):
                    for src_core_id in src_core_ids:
                        ...
                        for cmd in src_stage.mem_store_commands:
                            if isinstance(cmd, MCA_CompiledOperatorGraph.Command.MEM_STORE_TILE) and cmd.buf_name in common_buf_names and not cmd.is_partial:
