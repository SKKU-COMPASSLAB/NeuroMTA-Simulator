...
class MXUContext:
    def __init__(
        ...
        self._config = mxu_config
        
        self._dtype         = self._config.dtype
        self._acc_dtype     = self._config.acc_dtype
        
        # # Determine the tile shape
        if self._config.seq_len != self._config.pe_arr_height:
    ...
    def execute_gemm(self, ifm_tile: torch.Tensor, wgt_tile: torch.Tensor=None, psum_tile: torch.Tensor=None) -> torch.Tensor:
        ...
        if ifm_tile.shape != self.config.ifm_tile_shape:
            raise Exception(f"IFM tile shape {ifm_tile.shape} does not match expected shape {self.config.ifm_tile_shape}.")
        if wgt_tile.shape != self.config.wgt_tile_shape:
            raise Exception(f"WGT tile shape {wgt_tile.shape} does not match expected shape {self.config.wgt_tile_shape}.")
    ...
    def execute_maxpool(self, ifm_tile: torch.Tensor, psum_tile: torch.Tensor=None) -> torch.Tensor:
        ...
        if ifm_tile.shape != self.config.ofm_tile_shape:
            raise Exception(f"IFM tile shape {ifm_tile.shape} does not match expected shape {self.config.ofm_tile_shape}.")
    ...
    def ifm_tile_size(self) -> int:
        return self.config.ifm_tile_numel * self.dtype.itemsize
    
    @property
    def wgt_tile_size(self) -> int:
        return self.config.wgt_tile_numel * self.dtype.itemsize
    
    @property
    def ofm_tile_size(self) -> int:
        return self.config.ofm_tile_numel * self.acc_dtype.itemsize
