...
class HostAction:
    ...
    def execute(self) -> bool:
        try:
            ...
            logger.error(f"HostAction execution failed: {self}")
            raise e
        
        return True
    
    def __str__(self):
        return f"HostAction(type={self.action_type.name}, tokens={self.tokens})"
...
class HostContext(dict):
    ...
    def run_compiled_entry(self, entry: NetworkGraphEntry):
        ...
        with self:
            runtime = self.rt_get(entry.submodule if entry.node_type == NetworkGraphEntryType.COMPILED_GRAPH else entry.node.kind())
            ...
            for action in self.actions:
                if not action.execute():
                    raise Exception(f"HostAction execution failed: {action}")
...
class NetworkGraph:
    def __init__(self, module: torch.nn.Module, graph_ivars: list[torch.Value], graph_ovars: list[torch.Value], graph_nodes: list[torch.Node], host_context: HostContext, entries: list[NetworkGraphEntry]):
        self.module = module
    ...
    def from_trace(cls, module: torch.nn.Module, *dummy_inputs, host_context: HostContext, disable_lowering: bool=False, disable_toposort: bool=False):
        ...
        host_context[graph_ivars[0]] = module
        for idx, ivar in enumerate(graph_ivars[1:]):
            host_context[ivar] = dummy_inputs[idx]
        ...
        for node in graph_nodes:
            ...
            host_context.run_entry(entry, trace_mode=True)
        ...
        graph = cls(module, graph_ivars, graph_ovars, graph_nodes, host_context, entries)
    ...
    def lowering(self, context_name=None):
        ...
        var_rename_map = {vname: f"{context_name}::{vname}" for vname in self.host_context.keys()}
    ...
    def get_outputs(self):
        if len(self.graph_ovars) == 1:
            return self.host_context[self.graph_ovars[0]]
        return [self.host_context[o] for o in self.graph_ovars]
    
    def run_graph(self, *dummy_inputs, trace_mode: bool=False):
        self.host_context[self.graph_ivars[0]] = self.module
        for idx, ivar in enumerate(self.graph_ivars[1:]):
            self.host_context[ivar] = dummy_inputs[idx]
        
        for entry in self.entries:
            try:
                self.host_context.run_entry(entry, trace_mode=trace_mode)
            except Exception as e:
